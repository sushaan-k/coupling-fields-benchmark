# GSE313642 HCC PBMC CITE-seq held-patient protocol

## Confirmatory question and status

This one-shot experiment tests whether an A/B-context-conditioned exact
conditional coupling field learned from 23 hepatocellular-carcinoma (HCC)
...
`data/confirmation/gse313642_hcc/candidate_designation_v1.json` fixes the
...
Before designation, only the A-07-01 GEX and FB feature and barcode gzip
payloads were opened. No Matrix Market header or body was requested. Their
hashes and the observed 3,078-barcode axes are bound in the designation. The
two barcode sets were equal but deposited in different orders, so every
reduction joins by exact barcode text and never by row position. All nine
frozen marker names resolved uniquely on both A-07-01 feature axes. This
single-pair check does not satisfy the all-patient axis gate below.
...
`data/confirmation/gse313642_hcc/source_manifest_v1.json` binds the exact
filename and official byte count for all 210 selected individual members: 140
feature/barcode axes, 22 calibration matrices, 24 post-freeze pilot matrices,
12 held-GEX prediction matrices, and 12 post-prediction held-FB matrices. Its
allowed-stage field is normative; a file cannot be requested in another stage.
...
lexically smallest exact GEX/FB pair stem. This selects one paired sample for
each of 35 patients without using assay values.
...
with deposited IDs retaining the hyphen. The first five A patients and first
seven B patients are held; the next five A and seven B patients are pilot; all
others are calibration.

| Role | A patients | B patients |
|---|---|---|
| Calibration | A30, A12, A08, A32, A34 | B15, B23, B17, B04, B08, B03 |
| Pilot | A33, A04, A03, A35, A31 | B02, B07, B06, B05, B21, B01, B12 |
| Held | A05, A02, A07, A21, A36 | B11, B14, B13, B22, B18, B09, B10 |

The 11 calibration patients determine every configuration. The 12 pilot
patients are an untouched source-promotion set. The 12 held patients remain
outcome-blind until source promotion and frozen held predictions are public.

## Mandatory all-patient axis gate

Before any matrix request, open and fully CRC-check the feature and barcode
gzip streams for all 35 selected GEX/FB pairs. For every pair:
...
4. every axis length and feature type must agree with the later Matrix Market
   dimensions; and
5. the preflight must record the ordered-axis and set hashes, observed byte
...
`results/development/gse313642_hcc_calibration_selection_v1.json`. That
artifact contains the exact reduced calibration tables and destroyed-link
tables required for the later one-shot 23-patient refit, their hashes and
...
set are committed, annotated-tagged as `gse313642-hcc-v1-calibration`, pushed,
and verified byte-for-byte at the public remote. The pilot command verifies
that remote tag and its ancestry, then reads the 12 pilot patients once. It
...
23-patient source refit.

Against each mandatory comparator, primary promotion requires all three:

1. strictly lower patient-equal mean deviance; and
2. strictly lower deviance in at least 8 of 12 pilot patients; and
3. strictly lower mean deviance separately among the five A patients and the
   seven B patients.
...
committed, annotated-tagged as `gse313642-hcc-v1-source`, pushed, and remotely
...
`gse313642-hcc-v1-predictions`, pushed, and remotely verified before score
authorization.

The score authorization binds the remotely verified prediction bytes and is
itself committed, annotated-tagged as
`gse313642-hcc-v1-score-authorization`, pushed, and remotely verified. Only
...
This experiment confirms context-conditioned interaction transfer across HCC
...
screening rule without an inferential interval; the confirmatory claim rests
on the untouched held panel. With no exact loss ties, the 10-of-12 favorable
...
| Stage | Path |
|---|---|
| Candidate | `data/confirmation/gse313642_hcc/candidate_designation_v1.json` |
| Source manifest | `data/confirmation/gse313642_hcc/source_manifest_v1.json` |
...
| Axis preflight | `results/development/gse313642_hcc_axis_preflight_v1.json` |
| Calibration selection and reductions | `results/development/gse313642_hcc_calibration_selection_v1.json` |
| Pilot/source result and 23-patient refit | `results/development/gse313642_hcc_source_v1.json` |
| Held predictions | `results/gse313642_hcc_predictions_v1.json` |
| Held result | `results/gse313642_hcc_confirmation_v1.json` |
| Stage attempts and authorizations | `data/confirmation/gse313642_hcc/` |

The irreversible order is public candidate and implementation tags;
all-35-pair axis preflight and public preflight tag; calibration claim and
calibration-only reduction; public calibration tag; separate pilot claim,
source gate, and refit; public source tag; held-GEX prediction claim; public
prediction tag; public score-authorization tag; separate held-FB score claim;
terminal result. Each authorization binds the exact public bytes and SHA-256
values of every upstream artifact, runner, test, and transitive estimator
module. Every tag is annotated, pushed, and checked against the immutable
public remote before the next capability is consumed. Attempts are exclusive,
and any exception after claiming a stage writes the terminal record before
exit.
