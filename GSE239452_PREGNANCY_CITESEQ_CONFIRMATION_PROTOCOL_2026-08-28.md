...
**Pre-outcome amendment v1.2.** The v1.1 development authorization was published at commit `3c65cad936c600a3043fb057c2be61ef297d3b7d` but never exercised: no GSE239452 development attempt, reduction, pilot result, refusal, or numerical matrix access existed. A final red-team review identified a mutable handoff between reduction and the separate pilot-fitting command. Version 1.2 requires the exact 20-field donor-record schema and all count, margin, profile, access, manifest, and preflight invariants. The authorized `reduce-development` process now reduces all 15 development donors, validates those records, and completes pilot selection, scoring, gating, and refitting from the in-memory records before writing either artifact. Later artifact validation reconstructs the exact development records from the hash-bound sources and requires canonical equality, so a plausible profile change or a margin-preserving table change with a recomputed sibling digest still refuses. This validation rereads development-only frozen-marker values and never held values. The standalone pilot command and retry path are removed. The v1.1 authorization is superseded and must not be used. The scientific question, donor split, cell and state construction, estimator, grids, comparators, gates, and held boundary are unchanged.
...
Any exception after this invocation creates a terminal attempt record writes `results/development/gse239452_citeseq_terminal_refusal.json` and permanently closes the candidate. Attempt creation is exclusive; a competing invocation that does not own the attempt cannot write the refusal artifact. Existing attempt, reduction, pilot, prediction, score, or refusal artifacts prohibit a rerun. There is no independently callable pilot-fitting phase.
...
Within the same authorized process that reads and reduces the development donors, calibration-only selection fits the intact primary, destroyed-link control, strongest residual, and graph-zero diagnostic on all seven calibration donors. The process scores their frozen predictions on the eight pilot donors and completes the gate in memory before serializing the reduced or pilot artifact. Against each promotion comparator, `best_residual` and `destroyed_link`, promotion requires all of:
...
Development authorization binds the exact public bytes of the runner, preflight, both focused tests, this protocol, the designation, source manifest, metadata preflight, and three numerical library modules at an immutable protocol commit. The authorization file is itself verified at a later immutable commit before the development attempt record is written. The single authorized development process reads each development donor once, validates the exact record schema and cross-field invariants, and runs the pilot analysis directly on those in-memory records. Only after pilot fitting completes does it serialize the reduced artifact and its pilot result; the latter binds the exact reduced bytes. The reduced validator rejects undeclared fields, invalid types, altered marginal profiles, inconsistent tables, and access-record changes, then reconstructs all records from the preflight-hashed development H5ADs and requires canonical equality. A later margin-preserving table edit, even with a recomputed sibling digest, therefore refuses. The pilot validator then recomputes model selection, both promotion comparisons, the gate, and the all-development refit.
