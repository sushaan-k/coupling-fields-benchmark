...
fit_one <- function(x) {
  ...
  tryCatch({
    ...
    attempts <- character()
    candidates <- list()
    attempt <- function(optimizer, budget) {
      candidate <- tryCatch({
        ...
        attempts <<- c(attempts, sprintf("%s[%d]:mu=%.12g,tau2=%.12g,loglik=%.12g",
                                        optimizer, budget, result$mu, result$tau2, result$loglik))
        ...
        attempts <<- c(attempts, sprintf("%s[%d]:FAILED:%s", optimizer, budget, conditionMessage(e)))
        NULL
      })
