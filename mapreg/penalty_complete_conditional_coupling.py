"""Penalty-complete donor-heterogeneity-aware conditional binary coupling.
...
positive ``eta`` and either data support for every entity or positive ridge,
the Hessian quadratic form is

``sum_de v_de u_de^2 / D + eta_eff sum_de (u_de-w_e)^2``
``+ ridge_eff ||w||^2 + graph_eff w' L w``.

With positive ridge it vanishes only at ``u=w=0``, even for entities with no
informative donor table; therefore every finite fit is unique. This is a
...
def _validated_laplacian(laplacian: np.ndarray, size: int) -> np.ndarray:
    """Validate a PSD graph penalty and remove tolerated roundoff modes."""
...
def _validate_minimum_support(
    ...
    if isinstance(minimum_informative_donors, (bool, np.bool_)):
        raise ValueError("minimum_informative_donors must be an integer")
    try:
        minimum = operator.index(minimum_informative_donors)
    except TypeError as error:
        raise ValueError("minimum_informative_donors must be an integer") from error
    if minimum < 0 or minimum > donors:
        raise ValueError(
            "minimum_informative_donors must be between zero and donor count"
        )
    if minimum == 0 and (heterogeneity_penalty <= 0.0 or ridge_penalty <= 0.0):
        raise ValueError(
            "minimum_informative_donors=0 requires positive heterogeneity and ridge penalties"
        )
...
def evaluate_hierarchical_conditional_log_odds(
    ...
    *,
    ...
    _validate_minimum_support(
        support_count,
        len(records),
        minimum_informative_donors,
        heterogeneity,
        ridge,
    )
...
def fit_hierarchical_conditional_log_odds(
    ...
    *,
    ...
    complement, which are the factors used by the solver. Setting
    ``minimum_informative_donors=0`` is permitted only with positive ridge;
    unsupported entities are then identified entirely by the declared
    penalties and do not enter the information-normalization scales.
    """
    ...
    laplacian = _validated_laplacian(
        product_hypergraph_laplacian(first, second), entity_count
    )
