...
We derive the complete range of a binary association when only its two coarse
frequencies and a fine interaction are known. The range gives an attainable
worst-case prediction and an error floor that additional observations at the
same coarse resolution cannot remove. We then measure the value of
...
\subsection{A sharp limit after coarsening}
Suppose the reported query is the pair of binary states $U=f(x)$ and $V=g(y)$.
Assume that $S$ is known and transferable, and that the recipient reports only
$m_U=\Pr(U=1)$ and $m_V=\Pr(V=1)$, both strictly between zero and one. For a fine
rectangle with $f(x_0)=g(y_0)=0$ and $f(x_1)=g(y_1)=1$, define its contrast
\begin{equation}
 d=S(x_0,y_0)+S(x_1,y_1)-S(x_0,y_1)-S(x_1,y_0).
 \label{eq:contrast}
\end{equation}
Let $L$ and $H$ be the smallest and largest such contrasts. The closure of
compatible coarse log odds is exactly $[L,H]$. Thus the coarse relationship is
identified precisely when $L=H$; its sign is fixed when this interval lies on
one side of zero.

Let $P_L$ and $P_H$ be the two tables with the observed margins and log odds
$L$ and $H$. We measure loss by the Kullback--Leibler (KL) divergence from the
true table to its prediction. The unique minimax table $P_*$ is a mixture of these
endpoints satisfying $\KL(P_L\Vert P_*)=\KL(P_H\Vert P_*)$.
Additional file 1 gives
its explicit formula and proves that no estimator based only on independent
unpaired coarse observations can beat this floor, even with unlimited cells.
The information-radius argument uses classical minimax KL geometry
\cite{vanerven}; the resolution-specific ingredient is the sharp compatible
interval.
...
RNA profiles reveal $\rho$; their individual frequencies do not. This example
establishes ambiguity even when all marker frequencies are known; the sharp
interval assumes only the two queried frequencies are known.
...
The sharp interval assumes a known transferable interaction and two focal
frequencies; it is not the identified set for the all-marker benchmark.
The examples establish ambiguity, not its biological magnitude. Query-wise
minimax predictions need not form a coherent multivariate law. Interaction
drift, sampling error, and coarsening ambiguity remain distinct.
...
gain is descriptive, not causal mediation. The response expansion applies near
independence, without an accuracy claim at the fitted interaction strength.
Its query score predicts model discrepancy, not error against an arbitrary
biological population. The query-ranking advantage over direct source strength
