...
def analyze_tables(data: Mapping[str, np.ndarray]) -> dict:
    """Replay every fit and comparison using only the deposited aggregate tables."""
    source, held = data["source_rank_tables"], data["held_rank_tables"]
    donor_ids = data["held_donor_ids"]
    mask, threshold_mask = data["source_pair_mask"], data["threshold_pair_mask"]
    if not np.array_equal(mask, informative(source).sum(axis=0) >= 2):
        raise ValueError("rank pair mask differs from source-only eligibility")
    if not np.array_equal(threshold_mask, informative(data["source_threshold_tables"]).sum(axis=0) >= 2):
        raise ValueError("threshold pair mask differs from source-only eligibility")
    nulls = data["source_lineage_null_tables"]
    if nulls.shape != (PERMUTATIONS, *source.shape):
        raise ValueError("exactly eight source-table permutations are required")
    for null in nulls:
        if any(not np.array_equal(null.sum(axis=axis), source.sum(axis=axis)) for axis in (-1, -2)):
            raise ValueError("a null changed source endpoint margins")
    intact = _evaluate(source, held, mask, donor_ids, "hierarchical")
    repeats = []
    for repeat, null_tables in enumerate(nulls):
        result = _evaluate(null_tables, held, mask, donor_ids, "hierarchical")
        result.update(repeat=repeat, comparison=compare_donors(
            intact["donor_results"], result["donor_results"], f"lineage-null-{repeat}"
        ))
        repeats.append(result)
    threshold_methods = {
        method: _evaluate(data["source_threshold_tables"], data["held_threshold_tables"],
    ...
    return {
        ...
        "source_donors": len(source), "held_donors": len(held),
        "cells_per_donor": int(source[0, 0, 0].sum()), "markers": list(data["markers"]),
        ...
        "source_median_threshold": {
            "thresholds": data["thresholds"].tolist(), "state_rule": "raw ADT count > source pooled median",
...
def analyze(data: Mapping[str, np.ndarray]) -> dict:
    """Construct the fixed sensitivities from selected counts, then analyze their tables."""
    result = analyze_tables(sufficient_tables(data))
    result.update(donor_ids=list(data["donor_ids"]), sample_ids=list(data["sample_ids"]),
                  roles=list(data["roles"]))
    thresholds = np.asarray(result["source_median_threshold"]["thresholds"])
    result["source_median_threshold"].update(
        adt_positive_counts_by_donor=np.sum(data["adt_counts"] > thresholds[None, :, None], axis=2).tolist(),
        adt_threshold_ties_by_donor=np.sum(data["adt_counts"] == thresholds[None, :, None], axis=2).tolist(),
    )
    return result
...
def main() -> None:
    ...
    inputs.add_argument("--tables", type=Path)
    ...
    with np.load(args.counts or args.tables, allow_pickle=False) as archive:
        data = dict(archive)
    aggregates = sufficient_tables(data) if args.counts else data
    intact = _evaluate(aggregates["source_rank_tables"], aggregates["held_rank_tables"],
                       aggregates["source_pair_mask"], aggregates["held_donor_ids"], "hierarchical")
    ...
    result = analyze(data) if args.counts else analyze_tables(aggregates)
    result["original_loss_max_absolute_error"] = error
    result["bindings"] = {
        "sufficient_tables_sha256": hashlib.sha256(
            (args.tables or args.sufficient_tables).read_bytes()).hexdigest(),
    ...
    if args.counts:
        result["bindings"]["counts_sha256"] = hashlib.sha256(args.counts.read_bytes()).hexdigest()
